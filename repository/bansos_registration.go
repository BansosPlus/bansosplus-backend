...
func (r *BansosRegistrationRepositoryImpl) GetBansosRegisByUserID(id int, statuses []string) ([]*BansosRegistrationWithBansos, error) {
	...
	rows, err := r.db.Table("bansos_registrations").
		Select("bansos_registrations.id, bansos_registrations.name as user_name, bansos.id as bansos_id, bansos.name as bansos_name, bansos.type, bansos_registrations.status, bansos.image_url, bansos_registrations.created_at, bansos_registrations.updated_at").
		Joins("JOIN bansos ON bansos_registrations.bansos_id = bansos.id").
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.UserName, &item.BansosID, &item.BansosName, &item.BansosType, &item.Status, &item.ImageUrl, &item.CreatedAt, &item.UpdatedAt); err != nil {
			return nil, err
		}
